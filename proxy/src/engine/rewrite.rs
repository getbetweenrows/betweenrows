...
impl VisitorMut for PgCompatVisitor {
    ...
    fn pre_visit_expr(&mut self, expr: &mut Expr) -> ControlFlow<Self::Break> {
        ...
        if let Expr::Function(function) = expr {
            ...
            if name.0.len() > 1
                && let Some(last_ident) = name.0.last().cloned()
            {
...
mod tests {
    ...
    fn test_pg_table_gets_qualified() {
        ...
        assert!(
            rewritten.contains("pg_catalog.pg_class"),
            "Expected pg_class to be qualified with pg_catalog, got: {}",
            rewritten
        );
    }
    ...
    fn test_non_pg_table_unchanged() {
        ...
        let rewritten = parse_and_rewrite(sql);
        assert!(
            rewritten.contains("FROM users") && !rewritten.contains("pg_catalog"),
            "Expected users table to remain unqualified, got: {}",
            rewritten
        );
    }
    ...
    fn test_already_qualified_unchanged() {
        ...
        assert!(
            rewritten.contains("pg_catalog.pg_class"),
            "Expected pg_catalog.pg_class to remain as-is, got: {}",
            rewritten
        );
    }
    ...
    fn test_function_schema_stripped() {
        ...
        let rewritten = parse_and_rewrite(sql);
        assert!(
            rewritten.contains("pg_get_userbyid")
                && !rewritten.contains("pg_catalog.pg_get_userbyid"),
            "Expected function schema to be stripped, got: {}",
            rewritten
        );
    }
    ...
    fn test_unqualified_function_unchanged() {
        ...
        assert!(
            ...
            "Expected count(*) to remain unchanged, got: {}",
            rewritten
        );
    }
    ...
    fn test_multiple_pg_tables() {
        ...
        assert!(
            rewritten.contains("pg_catalog.pg_class"),
            "Expected pg_class to be qualified, got: {}",
            rewritten
        );
        assert!(
            rewritten.contains("pg_catalog.pg_namespace"),
            "Expected pg_namespace to be qualified, got: {}",
            rewritten
        );
    }
}
