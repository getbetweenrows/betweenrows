...
**P1 stories (42):** DS-03, DS-05, DS-06, DS-09, DS-12, DS-14, DS-15, DS-18, DS-19, DS-20, DS-21, MT-02, MT-03, MT-04, MT-08, MT-11, RE-02, RE-06, RE-09, RE-11, CX-01, CX-03, CX-06, CX-11, CX-12, AU-02, AU-04, AU-08, AU-10, AU-11, CC-07, CC-08, CC-09, CC-10, CO-08, CO-09, CO-10, INT-01, INT-02, INT-03, INT-04, DM-05

**P2 stories (remaining):** All HI-xx; DS-07, DS-08, DS-11, DS-13, DS-16, DS-17, MT-10, LN-01, AU-09, GW-01, AC-01, AC-02, JT-01, AB-02, TEST-01, PR-01, PB-01, CR-01, SIM-01, DF-01, DF-02, DF-03, DF-04, DF-05; MT-07, MT-09; RE-03–RE-05, RE-07, RE-08, RE-10; CX-02, CX-04, CX-05, CX-07–CX-10; AU-05–AU-07; CC-02, CC-04–CC-06; INT-05, INT-06; DM-01–DM-04
...
| **DS-18** | P1 | As a **Platform Admin**, I want to write a single row filter policy that resolves to a different value per user based on their attributes, instead of creating N hardcoded policies per region/department/clearance level. | Regional data isolation where 50 regions exist — creating 50 separate policies with hardcoded `region_id = 'EMEA'` is unworkable. A single policy with `region_id = {user.region}` scales to any number of regions. | Dynamic per-user value injection in filter expressions via ABAC. | **Original**: `SELECT * FROM orders;` (User has attribute `region = 'EMEA'`) <br><br> **Rendered**: `SELECT * FROM orders WHERE region_id = 'EMEA';` <br><br> **Why decision functions can't solve this**: Decision functions gate whether a policy fires (`fire: true/false`) but cannot inject values into the filter/mask expression. The expression `region_id = ???` needs a per-user value — only template variable expansion can provide this. |
| **DS-19** | P1 | As a **Compliance Officer** (Security & Compliance), I want to write a single mask expression that references user attributes to produce different masking levels per user, without maintaining separate policies per role tier. | Tiered masking: `CASE WHEN {user.clearance} >= 3 THEN salary ELSE 0.00 END` — one policy replaces N role-gated policies. Today this requires a separate `column_mask` policy per tier, each gated by a WASM decision function. | Per-user attribute interpolation in mask expressions via ABAC. | **Original**: `SELECT name, salary FROM employees;` (User has `clearance = 2`) <br><br> **Rendered**: `SELECT name, 0.00 AS salary FROM employees;` <br><br> **Original**: `SELECT name, salary FROM employees;` (User has `clearance = 4`) <br><br> **Rendered**: `SELECT name, salary FROM employees;` |
| **DS-20** | P1 | As a **Data Engineer** (Developer / Engineer), I want masked columns to produce deterministic output — the same input always produces the same masked value — so that JOINs, GROUP BY, and deduplication on masked columns still work correctly. | An analyst joins `customers` to `support_tickets` on `email`. If `email` is masked non-deterministically (e.g., random tokens), the JOIN produces zero matches. Deterministic masking (e.g., `SHA256(email \|\| salt)`) preserves referential integrity while hiding the raw value. | Deterministic/format-preserving masking. | **Original**: `SELECT c.id, t.subject FROM customers c JOIN support_tickets t ON c.email = t.customer_email;` <br><br> **Rendered (non-deterministic — broken)**: JOIN fails because `'a***@example.com'` != `'b***@example.com'` even though both are the same email. <br><br> **Rendered (deterministic — correct)**: `SELECT c.id, t.subject FROM customers c JOIN support_tickets t ON SHA256(c.email \|\| 'salt') = SHA256(t.customer_email \|\| 'salt');` — JOIN works because the hash is consistent. <br><br> **Note**: Today admins can write deterministic mask expressions manually (e.g., `SHA256(email)`), but there's no guidance, validation, or built-in deterministic masking mode. A `deterministic: true` flag on `column_mask` policies could auto-apply consistent hashing. |
| **DS-21** | P1 | As a **Security Architect** (Security & Compliance), I want to ensure that expressions involving masked or denied columns — such as `CONCAT(first_name, ' ', last_name)` or `first_name \|\| ' ' \|\| last_name` — still have masking applied to the underlying columns, so derived/computed values don't leak raw PII. | If `first_name` is masked as `'***'`, then `SELECT CONCAT(first_name, ' ', last_name)` should produce `'*** Smith'`, not `'Alice Smith'`. The mask must be applied at the scan level before any expressions reference the column. | Masking propagation through computed expressions. | **Original**: `SELECT first_name \|\| ' ' \|\| last_name AS full_name FROM customers;` (first_name is masked) <br><br> **Rendered**: `SELECT '***' \|\| ' ' \|\| last_name AS full_name FROM customers;` <br><br> **Note**: BetweenRows applies masks via `transform_up` at the `TableScan` level, so this should already work — but it needs explicit test coverage to confirm masking propagates through all expression types (CONCAT, CASE, COALESCE, arithmetic, window functions). |
...
| **MT-11** | P1 | As a **Consultant** (External / Partner), I belong to multiple organizations and need a single row filter policy to grant access to all of them, without creating separate policies per org. | A consultant works with orgs A, B, and C. Today `{user.tenant}` is a single scalar — you'd need 3 separate policies or 3 role assignments. A multi-value attribute like `{user.organizations}` expanding to `IN ('org-a', 'org-b', 'org-c')` handles this with one policy. | Multi-value template variable expansion in filter expressions. | **Original**: `SELECT * FROM orders;` (User has attribute `organizations = ['org-a', 'org-b', 'org-c']`) <br><br> **Rendered**: `SELECT * FROM orders WHERE organization_id IN ('org-a', 'org-b', 'org-c');` <br><br> **Why this is distinct from MT-06**: MT-06 describes the use case but assumes a static `IN` list. This story requires dynamic expansion of a multi-value user attribute at query time. |
...
| **RE-11** | P1 | As a **Security Admin** (Platform Admin), I want to write a row filter that traverses a relationship table to determine access — e.g., filter orders by checking the customer's tenant via a JOIN or subquery in the filter expression. | Tables without a direct tenant column (e.g., `order_items` has no `organization_id`) need to traverse `order_items → orders → customers` to verify tenant. Today, `filter_expression` is parsed as a DataFusion expression — it's unclear whether subqueries like `order_id IN (SELECT id FROM orders WHERE organization_id = {user.tenant})` are supported. If not, relationship-based row filtering is structurally impossible for tables lacking a direct foreign key to the tenant. | **Original**: `SELECT * FROM order_items;` (User's tenant is `org-abc`) <br><br> **Rendered**: `SELECT oi.* FROM order_items oi WHERE oi.order_id IN (SELECT id FROM orders WHERE organization_id = 'org-abc');` <br><br> **Open question**: Does the expression parser support subqueries? If not, this requires either (a) extending the parser, (b) a new `filter_join` policy type that declares join paths, or (c) requiring all tables to carry a denormalized tenant column. |
...
| **CX-11** | P1 | As a **Security Admin** (Platform Admin), I want to enforce a hard cap on the number of rows a query can return, rejecting or truncating results that exceed the limit. | Preventing bulk data exfiltration by capping result sets. Today, decision functions run before query execution and cannot inspect result size. This requires a post-execution enforcement hook. | **Original**: `SELECT * FROM customers;` (Result set: 50,000 rows, limit: 1,000) <br><br> **Rendered**: Error `"Query result exceeds 1,000 row limit. Add a WHERE clause to narrow results."` <br><br> **Why decision functions can't solve this**: Decision functions evaluate before query execution — they have no access to result row counts. This requires a new enforcement point after the upstream query returns but before results are sent to the client. |
| **CX-12** | P1 | As a **Security Analyst** (Analyst), I want per-user query rate limiting tracked across a time window, so a compromised account cannot dump an entire table across many small queries. | Preventing data exfiltration via repeated small queries. Requires stateful tracking (rows returned per user per hour) that persists across queries. | **Original**: `SELECT * FROM customers LIMIT 100;` (Repeated 500 times in one hour, cumulative 50,000 rows) <br><br> **Rendered**: Error `"Hourly row quota exceeded (50,000/10,000). Access restricted until next window."` <br><br> **Architectural note**: Requires a stateful counter (per-user, per-datasource, per-window) maintained across queries — a fundamentally different mechanism from the current stateless per-query policy evaluation. |
...
| **AU-10** | P1 | As a **Platform Admin**, I want an emergency "disable all policies" kill switch for a datasource that takes effect immediately, in case a misconfigured policy causes a production outage (e.g., a wildcard `table_deny` that blocks all tables). | A bad policy deployment blocks all queries for all users. The admin needs to restore access in seconds, not minutes. Disabling policies one-by-one is too slow. Switching `access_mode` to `open` doesn't help if deny policies are active. | **Action**: Admin clicks "Emergency Bypass" on datasource → all policy enforcement suspended → all connections rebuilt immediately → queries pass through unfiltered. <br><br> **Audit**: Emergency bypass activation is logged with actor, timestamp, and reason field. A banner appears in the UI indicating bypass is active. Auto-expires after a configurable window (e.g., 1 hour) to prevent permanent bypass. |
| **AU-11** | P1 | As a **Platform Admin**, I want to preview the effective permissions for a specific user on a specific datasource — showing which tables/columns they can see, which policies apply, and what the rewritten query would look like — without impersonating them or running actual queries. | Debugging "why can't user X see table Y?" is the most common support request. Today the admin must mentally trace through role memberships, policy assignments, priority resolution, and visibility computation. An effective-permissions preview makes this instant. | **Action**: Admin opens User → "Effective Permissions" tab → selects datasource → sees: visible tables, visible columns per table, active row filters, active masks, denied columns, denied tables, with source annotation (which policy/role produced each effect). <br><br> **Note**: `GET /users/{id}/effective-policies?datasource_id=X` exists but returns raw policy list. This story asks for the **computed result** — the resolved visibility after all priority/conflict resolution, not just the input policies. |
...
| **CC-08** | P1 | As a **Platform Admin**, I want a decision function to call an external system (e.g., verify a Jira ticket, check an approval workflow, query a policy-as-code engine) before allowing a policy to fire. | Today, WASM decision functions are sandboxed — no network access, no filesystem, no external calls. Some governance workflows require external validation (e.g., "only unmask if the user has an approved ticket in ServiceNow"). | **Current limitation**: WASM sandbox prevents HTTP calls. <br><br> **Options**: (a) Pre-populate external state into decision function config at assignment time (admin manually updates config). (b) Add a webhook/callback mechanism that the proxy calls before policy evaluation, injecting the result into the decision function context. (c) Allow decision functions to declare "external dependencies" resolved by the proxy before WASM execution. |
| **CC-09** | P1 | As a **Security Admin** (Platform Admin), I want to see a diff of what will change before enabling or disabling a policy — which users gain or lose access to which tables/columns — so I can assess the blast radius before committing. | Enabling a wildcard `table_deny` policy could accidentally block all tables for all users. Today there's no preview — the admin enables it and hopes for the best. Related to SIM-01 but focused on real-time preview, not historical simulation. | **Action**: Admin clicks "Preview Enable" on a policy → UI shows: "3 users will lose access to 5 tables", "12 columns will be hidden for role 'analyst'", etc. <br><br> **Implementation**: Compute `effective_visibility(before)` and `effective_visibility(after)` for all affected users, diff the results. |
| **CC-10** | P1 | As a **Security Admin** (Platform Admin), I want to control what happens when the upstream database adds a new column and it is discovered during catalog resync — specifically, I want the option to hide new columns by default until I explicitly approve them. | In `open` mode, a DBA adds a `credit_score` column → catalog resync discovers it → all users can immediately `SELECT credit_score` before any admin creates a policy. There is a window between discovery and policy creation where sensitive data is exposed. | **Options**: (a) Alert admins when new columns are discovered (notification in UI + audit log entry). (b) Add a datasource setting: `new_column_visibility: "visible"` (default, current behavior) or `"hidden_until_approved"` — new columns are auto-denied until admin reviews. (c) Document as an inherent trade-off of `open` mode and recommend `policy_required` for production. |
...
| **CO-04** | P1 | **Deny overrides permit** | CC-01 + DS-01 | **Original**: `SELECT id, name, ssn FROM customers;` (User has a permit policy with SSN masking on `customers`, but a deny policy blocks `customers` entirely for their role.) <br><br> **Rendered**: `SELECT id, name, ssn FROM customers WHERE 1=0;` (Deny wins; masking policy from DS-01 is never applied because the deny short-circuits.) |
...
| **CO-06** | P1 | **Role-based masking tiers on same query** | DS-05 composite | Two users run `SELECT id, name, ssn FROM customers WHERE id = 'cust-123';` simultaneously. <br><br> **Analyst**: `SELECT id, name, CONCAT('***-**-', RIGHT(ssn, 4)) AS ssn FROM customers WHERE id = 'cust-123';` <br><br> **Manager**: `SELECT id, name, ssn FROM customers WHERE id = 'cust-123';` (Same policy, different policies matched by role — masking tier varies per user context.) |
...
| **CO-08** | P1 | **Mask propagation through UNION** | DS-01 + multi-source | **Original**: `SELECT name, ssn FROM customers UNION ALL SELECT name, ssn FROM archived_customers;` <br><br> Both sides of the UNION must have SSN masked independently — a mask policy on `customers.ssn` must also apply to `archived_customers.ssn` if targeted. If only one side is masked, the UNION leaks raw PII from the other. |
| **CO-09** | P1 | **Column deny in SELECT expression** | DS-10 + expression | **Original**: `SELECT id, UPPER(ssn) AS upper_ssn FROM customers;` (SSN is denied via `column_deny`) <br><br> **Expected**: Error or column stripped — the deny must catch `ssn` even when wrapped in a function expression, not just in bare column references. The deny engine must trace column lineage through expressions. |
| **CO-10** | P1 | **Mask + ORDER BY interaction** | DS-01 + sorting | **Original**: `SELECT id, ssn FROM customers ORDER BY ssn;` (SSN is masked) <br><br> **Question**: Does `ORDER BY` use the raw or masked value? If raw, the ordering leaks information about the underlying data (e.g., all `***-**-0001` through `***-**-0010` appear in sequence, revealing the first digits). If masked, the sort order is meaningless but safe. Both behaviors should be documented and intentional. |

---

## 11. Integration & Compatibility

_Stories about how the proxy interacts with real-world client tools, connection patterns, and SQL dialect edge cases._

| ID | Priority | User Story | Use Case Example | SQL Example / Scenario |
| :-------- | :--- | :--- | :--- | :--- |
| **INT-01** | P1 | As a **Platform Admin**, I want policies to work correctly when BI tools (Metabase, Tableau, Superset) connect through the proxy using a shared service account and connection pool. | BI tools use a single database user for all dashboard viewers. Per-user policies don't differentiate because everyone appears as `bi_service_account`. The admin needs a way to pass the real end-user identity through the BI tool's connection. | **Scenario**: 50 dashboard viewers connect via Metabase → Metabase uses one pooled connection as `bi_user` → all viewers see the same data regardless of their actual role. <br><br> **Options**: (a) Support a `SET SESSION betweenrows.real_user = 'alice'` command that BI tools can issue per-query (similar to `SET ROLE` in PostgreSQL). (b) Support passing user identity via connection string parameters (`?options=-c betweenrows.user=alice`). (c) Support HTTP header-based identity (for future HTTP/MCP interface). (d) Accept this as a known limitation — recommend dedicated connections per user for sensitive data. |
| **INT-02** | P1 | As a **Developer** (Developer / Engineer), I want policies to work correctly with the PostgreSQL extended query protocol (Parse/Bind/Execute), not just simple query protocol. | ORMs (SQLAlchemy, ActiveRecord, Prisma) and JDBC drivers use the extended protocol by default. If the proxy only applies policies during simple query parsing, prepared statements bypass enforcement. | **Scenario**: Application sends `PREPARE stmt AS SELECT ssn FROM customers WHERE id = $1` → `EXECUTE stmt('cust-123')` — if the policy engine only inspects SQL text during the simple query flow, the prepared statement may execute unmasked. <br><br> **Note**: The pgwire crate handles both protocols. Verify that `on_query` (simple) and `do_describe`/`do_execute` (extended) both pass through the policy engine. This needs explicit test coverage with a JDBC or libpq client using prepared statements. |
| **INT-03** | P1 | As a **Data Engineer** (Developer / Engineer), I want to understand how NULL values interact with mask expressions, so I can write masks that handle NULLs gracefully instead of producing unexpected results. | `mask_expression: "'***-**-' \|\| RIGHT(ssn, 4)"` — if `ssn` is NULL, SQL string concatenation with NULL produces NULL. The masked output is NULL instead of a placeholder like `'***-**-****'`. This may break BI dashboards that expect a string. | **Example**: `SELECT name, ssn FROM customers WHERE ssn IS NULL;` <br><br> **Expected**: `name: 'Alice', ssn: '***-**-****'` (or NULL — but the behavior should be documented and consistent). <br><br> **Actual**: `name: 'Alice', ssn: NULL` (because `'***-**-' \|\| RIGHT(NULL, 4)` = NULL in SQL). <br><br> **Options**: (a) Document that mask expressions must handle NULLs explicitly (e.g., `COALESCE(ssn, '') → mask`). (b) Auto-wrap mask expressions in `CASE WHEN col IS NULL THEN NULL ELSE mask_expr END`. (c) Provide a `null_handling` option on column_mask: `"preserve"` (NULL stays NULL) or `"mask"` (NULL gets masked too). |
| **INT-04** | P1 | As a **Developer** (Developer / Engineer), I want masked columns to have predictable types in the schema metadata, so BI tools and ORMs don't break due to type mismatches. | A `salary DECIMAL(10,2)` column masked as `'***'` changes the column type from numeric to string. A BI tool that expects a number may crash or display errors. Similarly, `information_schema.columns` may report the original type while the actual data returns a different type. | **Scenario**: Tableau connects → reads `information_schema.columns` → sees `salary: DECIMAL` → issues `SELECT salary FROM employees` → receives `'***'` (string) → type error in visualization. <br><br> **Options**: (a) Mask expression must return the same type as the original column (enforce during policy creation). (b) Update `information_schema.columns` to reflect the masked type (string). (c) Support type-preserving masks: numeric → `0.00`, timestamp → `'1970-01-01'`, boolean → `false`. (d) Document the type behavior and recommend type-consistent mask expressions. |
| **INT-05** | P2 | As a **Platform Admin**, I want to understand how the proxy behaves when a PgBouncer or other connection pooler sits between clients and the proxy. | Connection poolers (PgBouncer in transaction mode) reuse server connections across clients. If the proxy builds per-user `SessionContext` at connection time, a pooled connection may serve User A's context to User B. | **Scenario**: PgBouncer in transaction mode → User A connects, gets context with `column_deny` on SSN → connection returned to pool → User B picks up same connection → User B sees User A's restricted schema (missing SSN) instead of their own. <br><br> **Note**: This is a known architectural constraint. The proxy identifies users at the pgwire handshake and builds per-connection context. Connection poolers that multiplex users onto shared connections break this model. Document the supported deployment topology: clients → proxy (direct) — no pooler between client and proxy. A pooler between proxy and upstream DB is fine. |
| **INT-06** | P2 | As a **Data Engineer** (Developer / Engineer), I want to understand how policies interact with multi-statement queries and implicit transactions. | Some SQL clients send multiple statements in one message: `SELECT * FROM orders; SELECT * FROM customers;`. Each statement may be subject to different policies (different tables). Does the proxy apply policies per-statement or per-message? | **Scenario**: Client sends `BEGIN; SELECT ssn FROM customers; SELECT salary FROM employees; COMMIT;` in one message. SSN is masked, salary is denied. <br><br> **Expected**: Each statement is independently rewritten. SSN is masked in statement 1, salary is denied in statement 2. The transaction boundary does not affect policy evaluation. <br><br> **Note**: The pgwire simple query handler processes statements sequentially. Verify that policy state doesn't leak between statements in the same message. |
