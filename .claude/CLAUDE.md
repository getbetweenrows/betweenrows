...
**The dependency flows one way.** Nothing in `proxy/`, `admin-ui/`, `migration/`, `scripts/`, `.githooks/`, `.github/`, or `docs/` may reference `docs-site/`. betweenrows developers never need to think about `docs-site/` when building features — that separation is the whole point.

**Exception for repo-root user-facing docs.** `README.md`, `SECURITY.md`, and `CONTRIBUTING.md` may link into `docs-site/` files. These are the files GitHub renders as the landing view of the repo; we want them self-contained so a reader browsing on GitHub gets a working set of links without external dependencies. This is a documentation affordance, not a development dependency — none of the code or design-time docs should ever follow this path.
