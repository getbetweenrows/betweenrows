...
### 73. Transitive tenancy bypass

**Vector**: A row filter policy references a column (`org`, `tenant_id`, `workspace_id`, `brand`, `region`, etc.) that isn't on every child table it targets. The scope column lives on a parent reachable via foreign key, but the rewriter injects the filter at every matching `TableScan`. Without transitive-join support, queries against child tables either (a) fail with `column 'org' not found` (today's fail-secure behavior — unusable on any normalized schema) or (b) if the rewriter walks the FK graph automatically, resolve through the wrong FK and let one tenant see another's rows. The problem class is called **transitive tenancy** in the semantic-layer literature; the mechanism we build is terminology-neutral and serves any scope column.

**Attacks**:
  1. **Querying a child table directly when the parent has the column** — `SELECT * FROM payments` where `payments` has only `order_id` and the filter `org = {user.tenant}` targets the datasource with `scope=all`. Without resolution the query errors; with naive resolution it could pick the wrong FK path.
  2. **Joining child→parent and relying on the parent's filter to cascade** — `SELECT * FROM orders o JOIN payments p ON o.id = p.order_id`. If the rewriter doesn't inject the parent join for `payments` itself, the user's outer join returns rows filtered by `orders.org` but unfiltered at `payments`, leaking rows that belong to other tenants' payments but join to some rows of the current tenant's orders.
  3. **Mis-designated column anchor** — admin picks the wrong `table_relationship` as the anchor for a scope column (e.g. `payments.payer_id → users` instead of `payments.order_id → orders`). Queries resolve `org` through the wrong parent chain and return a different tenant's rows.
  4. **Column-name skew across tables in the same broad policy** — one row-filter policy says `tenant_id = {user.tenant}` and broadly targets a schema. Two tables under the policy spell the scope column differently: `customers.tenant_id` and `accounts.org_id`. Before same-table alias anchors, the filter would deny-wins on `accounts` even though the column exists under a different name; admins either fragmented the policy or used name-matched wrappers.

**Defense**: Admin-curated `table_relationship` catalog (FK tuples, child → parent) plus per-`(child_table, resolved_column)` `column_anchor` routing annotations. Anchors come in two XOR-exclusive shapes: (a) **FK walk** — `relationship_id` set, the rewriter follows the chain to a parent that carries the column literally; (b) **Same-table alias** — `actual_column_name` set, the rewriter rewrites the filter expression in place (`tenant_id` → `org_id`) with no join. DB-enforced uniqueness on `(datasource, child_table, resolved_column_name)` (partial unique index) guarantees exactly one route per (table, column) — the rewriter is deterministic by construction, not by BFS tie-breaking. For the FK-walk shape the rewriter walks anchors from the target table, following `relationship_id` to the parent; if the parent doesn't carry the column, it looks up the next anchor from that parent and continues up to a hardcoded depth of 3. The emitted plan is `Project([target.*], Filter(rewritten, InnerJoin(target, parent_chain)))` — INNER JOIN only. For the alias shape the rewriter collects a `resolved_column → actual_column_name` rename map and applies it inside `requalify_expr`; no join, no top projection (the target scan already exposes the actual column). The `parent_column_name` is expected to be a PK or single-column unique so each child row matches at most one parent (at-most-one-parent-per-child → row-count-safe); see the Status section for the trust assumption on that invariant. A `Project(target.schema().columns())` at the top re-emits exactly the target scan's output columns — sourced from the actual `LogicalPlan` schema rather than from `columns_by_table`, so partial projections (`SELECT id, amount FROM payments`), zero-column projections (DF 52+ `SELECT COUNT(*)` optimization), and column-mask-wrapped scans all round-trip without "field not found" errors. Downstream plan nodes (including the mask-above-scan projection at `proxy/src/hooks/policy.rs:1994`) see a byte-for-byte-identical schema. Five failure modes all produce deny-wins (`lit(false)` filter → zero rows, structured `tracing::warn!(reason="column_resolution_unresolved", ...)` for ops visibility): (1) no `column_anchor` on the target or an intermediate hop; (2) an FK-walk chain that exceeds `MAX_DEPTH = 3`; (3) a chain cycle (the resolver detects via a `visited` set); (4) a qualified column reference to a non-target table in the filter (`WHERE orders.org = ...`), which v1 does not support; (5) the filter references a column that is neither on the `TableScan`'s output schema nor resolvable via any anchor — this also catches alias anchors whose `actual_column_name` points at a non-existent column (save-time does not validate this per design; query-time is the single source of truth). The 5th check happens in `apply_row_filters` against the actual scan schema (not the snapshot) — so partial discoveries, race conditions between discovery and policy evaluation, and any other gap that would otherwise surface as a DataFusion "field not found" plan error are converted into deny-wins with an audit-visible warn log. NULL FKs drop out automatically via INNER JOIN ON semantics (no special-casing). See `proxy/src/resolution/graph.rs` for the algorithm and `proxy/src/admin/relationship_handlers.rs` for the admin CRUD + live `pg_constraint` FK suggestion path.

**Previously**: Row filters were injected blindly via `transform_up` above every matching `TableScan`. If the filter referenced a column the target didn't carry, DataFusion's planner errored with `column 'X' not found` at plan time. Fail-secure (no rows leaked) but unusable on any normalized schema — customers whose scope column lived on a parent table couldn't apply scope-isolating row filters at all.

**Status**: **Accepted trade-off** on attack 3, with a secondary trade-off on the `parent_column_name` invariant. Mis-designated column anchors are the load-bearing trust assumption of this design and cannot be fully mitigated by the proxy — admin intent is not inferable from the schema. Mitigations: (1) the admin UI shows every FK candidate returned by live `pg_constraint` introspection (filtered to PK or single-column unique parents) so admins see alternatives before choosing, (2) the partial unique index enforces at-most-one anchor per (child_table, resolved_column), removing nondeterministic path selection, (3) all relationship and anchor mutations go through `admin_audit_log` and surface in `AuditTimeline`, and each `create_relationship` emits a structured `tracing::info!` describing the trust assumption so operators can correlate logs with the audit trail. **Parent-column PK/unique is not server-verified on the `POST /datasources/{id}/relationships` path** — only the UI-facing `GET /datasources/{id}/fk-suggestions` endpoint filters by PK/unique. If an admin directly POSTs a relationship whose `parent_column_name` is not unique, the injected INNER JOIN will fan child rows and corrupt row-filter arithmetic (row duplication and potential cross-tenant rows after filter evaluation). This is accepted as a documented trade-off: adding a server-side PK/unique check would require persisting uniqueness state on `discovered_column` and keeping it in sync with upstream schema changes — complexity we chose not to take on in v1. Out-of-band review of column anchor designations is recommended for any datasource carrying sensitive tenant data. A v2 improvement will add an anchor-mutation alerting channel and may re-introduce server-side PK/unique verification if the operational cost of out-of-band review proves too high.

**Tests**:
  - `resolution::graph::tests::single_hop_resolves` (unit) — attack 1 (single-hop resolution injects INNER JOIN, projects back to target schema)
  - `resolution::graph::tests::multi_hop_resolves` (unit) — attack 1 (chain walks multiple hops)
  - `resolution::graph::tests::mixed_target_and_parent_refs_handled` (unit) — attack 2 (filter referencing a target-resident column alongside a parent-resolved column rewrites cleanly)
  - `resolution::graph::tests::projection_preserves_target_schema_only` (unit) — attack 1 (no column leakage through the top projection; schema-level assertion on `plan.schema()` field set and qualifier table)
  - `resolution::graph::tests::partial_target_projection_emits_only_scanned_columns` (unit) — partial TableScan projections round-trip without "field not found" errors
  - `resolution::graph::tests::zero_column_target_scan_resolves_without_error` (unit) — DF 52+ `SELECT COUNT(*)` zero-column scan path doesn't emit an invalid Projection
  - `resolution::graph::tests::no_anchor_returns_error` (unit) — attack 3 (missing anchor triggers deny-wins at the caller, not silent path selection)
  - `resolution::graph::tests::qualified_parent_ref_rejected` (unit) — a v1 guard against ambiguous qualified-parent references in filter expressions
  - `resolution::graph::tests::cycle_detected` (unit) — chain cycle produces deny-wins without infinite recursion
  - `resolution::graph::tests::depth_limit_exceeded_when_chain_too_long` (unit) — chains longer than `MAX_DEPTH` produce deny-wins
  - `resolution::graph::tests::fast_path_when_column_present_on_target` (unit) — resolution is skipped (no joins injected) when every referenced column is target-resident
  - `resolution::graph::tests::fast_path_when_snapshot_has_no_target_columns` (unit) — resolver defers to `apply_row_filters`' scan-schema check when no catalog info exists for the target (5th failure mode hand-off)
  - `hooks::policy::tests::row_filter_deny_wins_when_column_not_on_scan_and_no_anchor` (unit) — 5th failure mode: scan-schema check substitutes `Filter(lit(false))` instead of letting a DF plan error through
  - `hooks::policy::tests::precompute_parent_scans_serves_from_session_cache` (unit) — `SessionData.parent_scans_cache` is read before the session catalog; cross-query amortization
  - `policy_enforcement::vector_73_happy_path_row_filter_via_parent_anchor` (integration) — attack 1 (happy path: FK + anchor + row filter referencing parent column → only correctly scoped rows returned through the real proxy)
  - `policy_enforcement::vector_73_deny_wins_when_no_anchor_configured` (integration) — attack 3 / 5th failure mode (missing anchor → zero rows end-to-end, not a plan error)
  - `policy_enforcement::vector_73_cache_invalidation_on_new_anchor` (integration) — POSTing a new anchor takes effect on an open session without reconnect (proves `invalidate_datasource` drops the cached snapshot + parent-scan cache)
  - `policy_enforcement::vector_73_cross_datasource_column_anchor_rejected` (integration) — handler rejects anchor whose `relationship_id` belongs to a different datasource with 404
  - `resolution::graph::tests::alias_anchor_rewrites_filter_column_name` (unit) — attack 4 (same-table alias substitutes column name in filter expression, no join)
  - `resolution::graph::tests::alias_anchor_contributes_no_parent_scans` (unit) — attack 4 (alias anchors never trigger parent-scan preplanning)
  - `resolution::graph::tests::mixed_alias_and_fk_walk_in_one_filter` (unit) — attack 4 (same filter with both alias and FK-walk columns resolves correctly)
  - `resolution::graph::tests::alias_preferred_over_fk_walk_when_both_keys_would_match` (unit) — `AnchorShape` discriminator correctness
  - `admin::dto::tests::column_anchor_request_relationship_shape` (unit) — FK-walk DTO round-trips
  - `admin::dto::tests::column_anchor_request_alias_shape` (unit) — alias DTO round-trips
  - `admin::dto::tests::column_anchor_request_neither_shape_parses_but_handler_rejects` (unit) — DTO is permissive; handler enforces XOR
  - `admin::dto::tests::column_anchor_request_both_shapes_parse_but_handler_rejects` (unit) — DTO accepts both; handler rejects
  - `policy_enforcement::tc_alias_anchor_basic` (integration) — attack 4 (one broad policy, two tables with different column names, both filter correctly end-to-end)
  - `policy_enforcement::tc_alias_anchor_column_typo_hits_deny_wins` (integration) — alias pointing at a non-existent column produces deny-wins via the 5th failure mode at query time (no save-time catalog check per design)
  - `policy_enforcement::tc_alias_anchor_save_xor_validation` (integration) — POST with neither / both / empty `actual_column_name` returns 422
  - `policy_enforcement::tc_alias_plus_fk_walk_coexist` (integration) — one datasource with one anchor of each shape, both driving the same broad policy
  - `policy_enforcement::tc_alias_anchor_cache_invalidation` (integration) — creating an alias anchor mid-session takes effect on the next query without reconnect
