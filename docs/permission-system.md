...
#### Filtering by a column that isn't on the matched table

A `filter_expression` may reference a column that isn't on the matched table directly. `column_anchor` rows tell the row-filter rewriter how to resolve the reference. An anchor is one of two shapes:

- **FK walk** (`relationship_id` set) — the column lives on a parent table reachable via an admin-registered `table_relationship`. Example: `org = {user.tenant}` applied to `payments` when `org` lives on `orders` and `payments.order_id → orders.id`. At query time the proxy walks the anchor chain up to the parent (up to 3 hops if the column still isn't present) and rewrites the plan to `Project([target.*], Filter(expr, InnerJoin(target, parent_chain)))`. The top projection re-emits exactly the target's scan schema, so downstream plan nodes (including column masks) are unaffected. INNER JOIN semantics drop child rows whose parent FK is NULL, which matches the intent of tenant/scope isolation.

- **Same-table alias** (`actual_column_name` set) — the column lives on the target table itself but under a different name. Example: one broad row-filter policy says `tenant_id = {user.tenant}`; on `customers` the field is literally `tenant_id`, but on `accounts` it's called `org_id`. Designating `actual_column_name: "org_id"` for the `tenant_id` anchor on `accounts` tells the rewriter to substitute `tenant_id → org_id` in the filter expression — no join is built. Alias anchors are used when the semantic column exists on the target but the spelling differs.

Exactly one of `relationship_id` / `actual_column_name` must be set per anchor (XOR). Exactly one anchor per `(child_table, resolved_column)` is allowed — the DB enforces this via a partial unique index, so resolution is deterministic by construction (no BFS tie-breaking at query time).

For the FK-walk shape, an admin must also register the foreign key as a `table_relationship` first. The admin UI surfaces live FK candidates via `GET /datasources/{id}/fk-suggestions` (only single-column FKs whose parent column is a PK or single-column unique are suggested). Promoting a suggestion creates a `table_relationship` row describing the child→parent edge. For the alias shape, no relationship is needed.

**Deny-wins on resolution failure.** If no anchor is configured for a referenced column, or an FK walk exceeds 3 hops, or a cycle is detected, or an alias anchor points at a column that doesn't exist on the scan, or the column can't be resolved for any other reason, the row filter is replaced with `Filter(false)` — zero rows returned, not a query error — and a structured `tracing::warn!` is emitted for ops visibility. Save-time does not validate that `actual_column_name` exists in the discovered catalog; the query-time check is the single source of truth.

**Admin intent is load-bearing.** The anchor designation is the trust boundary: a mis-designated anchor (FK-walk one pointing at the wrong parent, or an alias pointing at an unrelated local column) can return a different tenant's rows than the admin intended. The proxy cannot infer intent from the schema alone. Mitigations: the UI shows every FK candidate and the table's own columns (via datalist autocomplete) so admins see alternatives before choosing; anchor/relationship mutations go through the admin audit log and `AuditTimeline`; every `create_relationship` emits a structured `tracing::info!` describing the trust assumption. See vector 73 in `docs/security-vectors.md` for the full attack/defense write-up.

**Edit-time coverage preview.** Because resolution failures fail-secure (zero rows, no error), an admin can write a perfectly valid-looking row filter, assign it to several tables, and have it silently deny on every table that lacks a matching anchor — discoverable today only by running a query and noticing the empty result, or by grepping the proxy logs for `column_resolution_unresolved`. To close that authoring gap the admin API exposes `GET /policies/{id}/anchor-coverage`, which dry-runs the same resolution logic against every (assigned table × column referenced in the filter expression) and returns one of five verdicts per pair: `on_table`, `anchor_walk` (with the FK edge metadata), `anchor_alias`, `missing_anchor` (the silent-deny case), or `missing_column_on_alias_target`. The admin UI consumes this endpoint on the policy edit page and renders a green banner when every pair resolves or a red panel listing the broken pairs with a link to add the missing anchor on the datasource page. The endpoint is read-only, scoped per policy, and reuses the same in-memory `RelationshipSnapshot` the runtime uses, so the verdict matches runtime behavior exactly. Non-row-filter policy types return an empty coverage array (the panel hides itself).
